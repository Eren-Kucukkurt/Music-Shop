...
const Checkout = () => {
    ...
    const fetchCart = async () => {
      try {
        ...
        const items = response.data.items.map(item => ({
          id: item.id,
          productId: item.product.id, // Extract product ID
          productName: item.product.name, // Extract product name
          quantity: item.quantity,
          price: parseFloat(item.price), // Ensure price is treated as a number
          totalPrice: parseFloat(item.total_price), // Ensure total price is a number
        }));
        setCartItems(items);
        setLoading(false);
      } catch (err) {
  ...
  const handleCheckout = async () => {
    try {
      ...
      // Redirect to orders or confirmation page after a delay
      setTimeout(() => {
        navigate('/orders'); // Replace with your orders or confirmation route
      }, 3000);
    } catch (err) {
